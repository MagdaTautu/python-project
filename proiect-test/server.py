        ...
        while True:
                ...
                if guess.isdigit():
                    ...
                    if guess == num_to_guess:
                        client_socket.sendall("Correct! You guessed it! GAME OVER!".encode())
                        print("Client guessed the number!")
                        for sock in client_sockets:
                            sock.close()
                        break
                    else:
                        client_sockets[client_number].sendall("Try again. Your turn: ".encode())
    ...
    if num_players == 2:
        ...
        if mode == 1:  # Server generates the number
            if client_number == 0:
                num_to_guess = generate_number() 
                print(f"The server generated the number {num_to_guess}")
                client_socket.sendall("Your turn. Please guess a number! ".encode())
            else:
                client_socket.sendall("Wait for the other player to guess.".encode())
            ...
            while over == False:
                ...
                if guess.isdigit():
                    ...
                    if guess == num_to_guess:
                        ...
                        client_sockets[other_client].sendall(f"You lost! The other player guessed it! GAME OVER! ".encode())
                        over = True
                        for sock in client_sockets:
                            sock.close()
                        break
                        ...
                        if guess < num_to_guess:
                            client_sockets[client_number].sendall("Try a higher number. Wait for the other player.".encode())
                        else: client_sockets[client_number].sendall("Try a smaller number. Wait for the other player.".encode())
                        other_client = 1 - client_number
                        client_sockets[other_client].sendall(f"Client {client_number + 1} guessed.".encode())
                        client_sockets[other_client].sendall("Your turn. Please guess a number! ".encode())
